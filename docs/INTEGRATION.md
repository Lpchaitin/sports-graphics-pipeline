...
```bash
# In your model repo
python /path/to/sports-graphics-pipeline/scripts/process_picks.py \
...
subprocess.run([
    'python',
    '/path/to/sports-graphics-pipeline/scripts/process_picks.py',
    'picks.csv',
...
jobs:
  generate:
    ...
    steps:
      ...
      - name: Generate graphics
        run: |
          python sports-graphics-pipeline/scripts/process_picks.py \
            output/picks.csv \
            --output-dir graphics/mlb
...
0 9 * * * python /path/to/sports-graphics-pipeline/scripts/process_picks.py \
...
result = subprocess.run([
    'python',
    '../sports-graphics-pipeline/scripts/process_picks.py',
...
python scripts/process_picks.py examples/example_picks.csv --output-dir output/mlb/test
...
**Missing logos?**
- Run `python scripts/download_mlb_logos.py` to download all logos with white outlines
...
- `examples/example_picks.csv` - Sample CSV format
- `scripts/generate_sample.py` - Single game example
- `scripts/process_picks.py` - Batch processing script
